...
if [ "$1" == "editor" ]; then
  ...
  cp "$GODOT_BIN_DIR/godot.x11.opt.tools.64" "$ARTIFACTS_PATH"
fi
...
if [ "$1" == "templates" ]; then
  ...
  cp "$GODOT_BIN_DIR/godot.x11.opt.debug.64" "$ARTIFACTS_PATH"
  cp "$GODOT_BIN_DIR/godot.x11.opt.64" "$ARTIFACTS_PATH"
fi
